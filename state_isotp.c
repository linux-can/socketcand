	...
	while (previous_state != STATE_ISOTP) {
		...
		if (!strncmp("< isotpconf ", buf, 12)) {
			items = sscanf(buf, "< %*s %x %x %x "
					"%hhu %hhx %hhu "
					"%hhx %hhx %hhx %hhx >", &addr.can_addr.tp.tx_id, &addr.can_addr.tp.rx_id, &opts.flags, &fcopts.bs, &fcopts.stmin, &fcopts.wftmax,
					&opts.txpad_content, &opts.rxpad_content, &opts.ext_address, &opts.rx_ext_address);
			...
			if ((opts.flags & CAN_ISOTP_RX_EXT_ADDR && items < 10) || (opts.flags & CAN_ISOTP_EXTEND_ADDR && items < 9)
					|| (opts.flags & CAN_ISOTP_RX_PADDING && items < 8) || (opts.flags & CAN_ISOTP_TX_PADDING && items < 7) || (items < 5)) {
